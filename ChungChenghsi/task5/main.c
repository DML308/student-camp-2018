...
struct ast * newfunc(int functype, struct ast *l) {
    struct fncall * a = malloc(sizeof(sturct fncall));
...
struct ast * newref(struct symbol * s) {
    struct symref * a = malloc(sizeof (sturct symref));
...
struct ast * newflow(int nodetype, struct ast * cond, struct ast *tl, struct *el) {
...
static double callbuiltin(struct fncall *f) {
    ...
    switch(functype) {
        ...
        default: yyerror("Unknown built-in function %d", functype); return 0;
    }
}
